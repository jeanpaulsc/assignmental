...
def show():
    ...
    db.post.concept_id.default = this_concept.id
    form = SQLFORM(db.post).process() if auth.user else None
    conceptcomments = db(db.post.concept_id==this_concept.id).select()
    return dict(concept=this_concept, comments=conceptcomments, form=form)
...
def documents():
	...
	db.document.concept_id.default = concept.id
	db.document.concept_id.writable = False
	grid = SQLFORM.grid(db.document.concept_id==concept.id,args=[concept.id]) 
	return dict(concept=concept, grid=grid)
