...
def create():
    ...
    form = SQLFORM(db.thread).process(next=URL('root'))
...
def spread():
	"""expands thread hierarchy of children"""
	this_thread = db.thread(request.args(0,cast=int)) or redirect(URL('index'))
	db.node.thread_id.default = this_thread.id
	form = SQLFORM(db.post).process() if auth.user else None
	threadnodes = db(db.node.thread_id==this_thread.id).select()
	return dict(thread=this_thread, nodes=threadnodes, form=form)
