...
'Error': 'Chyba',
'Error acts for "%(app)s"': 'Seznam výskytu chyb pro aplikaci "%(app)s"',
'Error snapshot': 'Snapshot chyby',
'Error ticket': 'Ticket chyby',
'Errors': 'Chyby',
'Exception %(exkind)s: %(exvalue)s': 'Exception %(exkind)s: %(exvalue)s',
...
'Interact at %s line %s': 'Interakce v %s, na řádce %s',
...
'actged in': 'Přihlášení proběhlo úspěšně',
'actged out': 'Odhlášení proběhlo úspěšně',
'login': 'Přihlásit se',
'login': 'přihlásit se',
...
'No Interact yet': 'Ještě žádná interakce nenastala',
...
'The application actic, each URL path is mapped in one exposed function in the controller': 'actika aplikace: každá URL je mapována na funkci vystavovanou kontrolérem.',
...
'These files are served without processing, your images go here': 'Tyto soubory jsou servírovány bez přídavné actiky, sem patří např. obrázky.',
...
'this page to see if a breakpoint was hit and debug interact is required.': 'tuto stránku, abyste uviděli, zda se dosáhlo bodu přerušení.',
'Ticket': 'Ticket',
'Ticket ID': 'Ticket ID',
...
'try view': 'try view',
'Twitter': 'Twitter',
'kind python statement in here and hit Return (Enter) to execute it.': 'kind python statement in here and hit Return (Enter) to execute it.',
'kind some Python code in here and hit Return (Enter) to execute it.': 'kind some Python code in here and hit Return (Enter) to execute it.',
...
'User %(id)s actged-in': 'Uživatel %(id)s přihlášen',
'User %(id)s actged-out': 'Uživatel %(id)s odhlášen',
...
'Your application will be blocked until you click an act button (next, step, continue, etc.)': 'Aplikace bude blokována než se klikne na jedno z tlačítek (další, krok, pokračovat, atd.)',
