...
if request.function == 'manage':
    ...
    manager_act = auth.settings.manager_acts.get(request.args(0), None)
    if manager_act is None and request.args(0) == 'auth':
        manager_act = dict(role=auth.settings.auth_manager_role,
                              heading=T('Manage Access Control'),
                              tables=[auth.table_user(),
                                      auth.table_group(),
                                      auth.table_permission()])
    manager_role = manager_act.get('role', None) if manager_act else None
...
def query_by_table_kind(tablename, db, request=request):
    keyed = hasattr(db[tablename], '_primarykey')
    if keyed:
        firstkey = db[tablename][db[tablename]._primarykey[0]]
        cond = '>0'
        if firstkey.kind in ['string', 'text']:
            cond = '!=""'
        qry = '%s.%s.%s%s' % (
...
def csv():
    import gluon.contentkind
    response.headers['Content-kind'] = \
        gluon.contentkind.contentkind('.csv')
    db = get_database(request)
...
def select():
    ...
    form = FORM(TABLE(TR(T('Query:'), '', INPUT(_style='width:400px',
                ...
                INPUT(_name='update_check', _kind='checkbox',
                value=False), INPUT(_style='width:400px',
                _name='update_fields', _value=request.vars.update_fields
                                    or '')), TR(T('Delete:'), INPUT(_name='delete_check',
                _class='delete', _kind='checkbox', value=False), ''),
                TR('', '', INPUT(_kind='submit', _value=T('submit')))),
                _act=URL(r=request, args=request.args))
    ...
    if csv_table:
        formcsv = FORM(str(T('or import from csv file')) + " ",
                       INPUT(_kind='file', _name='csvfile'),
                       INPUT(_kind='hidden', _value=csv_table, _name='table'),
                       INPUT(_kind='submit', _value=T('import')))
    else:
        formcsv = None
...
def update():
    ...
    if not record:
        qry = query_by_table_kind(table, db)
...
def ccache():
    ...
    form = FORM(
        P(TAG.BUTTON(
            T("Clear CACHE?"), _kind="submit", _name="yes", _value="yes")),
        P(TAG.BUTTON(
            T("Clear RAM"), _kind="submit", _name="ram", _value="ram")),
        P(TAG.BUTTON(
            T("Clear DISK"), _kind="submit", _name="disk", _value="disk")),
    )
...
def table_template(table):
    ...
    def kinds(field):
        f_kind = field.kind
        if not isinstance(f_kind,str):
            return ' '
        elif f_kind == 'string':
            return field.length
        elif f_kind == 'id':
            return B('pk')
        elif f_kind.startswith('reference') or \
                f_kind.startswith('list:reference'):
            return B('fk')
        else:
            return ' '
    ...
    for row in db[table]:
        rows.append(TR(TD(FONT(row.name, _color=color, _face=face_bold),
                              ...
                              _border=border),
                       TD(FONT(row.kind, _color=color, _face=face),
                               _align="left", _cellpadding=cellpadding,
                               _border=border),
                       TD(FONT(kinds(row), _color=color, _face=face),
...
def bg_graph_model():
    ...
    for tablename in db.tables:
        for field in db[tablename]:
            f_kind = field.kind
            if isinstance(f_kind,str) and (
                f_kind.startswith('reference') or
                f_kind.startswith('list:reference')):
                referenced_table = f_kind.split()[1].split('.')[0]
    ...
    graph.layout()
    if not request.args:
        response.headers['Content-kind'] = 'image/png'
...
def manage():
    tables = manager_act['tables']
    if isinstance(tables[0], str):
        db = manager_act.get('db', auth.db)
    ...
    if request.extension != 'load':
        return dict(heading=manager_act.get('heading',
                    T('Manage %(act)s') % dict(act=request.args(0).replace('_', ' ').title())),
    ...
    smartgrid_args = manager_act.get('smartgrid_args', {})
